...
I also increasingly really do not enjoy sharing things on social media. The inclusion of the "updates" feed will hopefully allow me to put small things, which may only have one image or one link associated with them (maybe even just text?) in a place that I own, both for my archive and for interested people.
...
**The list-view slideshow:**

The right-hand panel in list view cycles through media pulled from the markdown files. What appears there is controlled entirely by frontmatter:

```yaml
display:
  coverImage: "/images/smt/smt.mp4"    # used as the slide if slideshowImages is empty
  slideshowImages:                      # optional, overrides coverImage
    - "/images/smt/smt-1.jpg"
    - "/images/smt/smt-2.jpg"
  showInSlideshow: true                 # this is the switch — must be true
```

- `showInSlideshow: true` is the only thing that puts an item in the rotation. An item with slideshow images but `showInSlideshow: false` will not appear (its media still shows on hover).
- If `slideshowImages` is empty, the item's `coverImage` is used. So the minimum setup is a `coverImage` plus `showInSlideshow: true`.
- **Videos work anywhere images do.** `.mp4`, `.webm`, and `.mov` become autoplaying muted loops; anything else becomes an `<img>`. Video slides restart from the beginning each time they come round.
- `display.slideshowLinks` adds live iframe embeds as extra slides for the same item.

Order is deterministic and matches the list on the left: **items tagged `selected` first, then most recent first by `date`.** Within one item, `slideshowImages` play in the order written, followed by any `slideshowLinks`. Each slide holds for 5 seconds. Hovering a list item pauses the rotation and jumps the panel to that item's media; moving away resumes where it left off.
...
`utilities/` holds scripts that are run by hand, not during the build. `optimize-media.js` re-encodes the source images and videos in `src/images/` in place — see [utilities/README.md](utilities/README.md). Run `npm run optimize` for a report and `npm run optimize:apply` to write.
